...
uint8_t my_opt_memmove(uint8_t * src, uint8_t * dst, uint32_t length)
...
uint8_t  my_opt_memset(uint8_t * src, uint32_t length, uint8_t value)
{
...
uint8_t  my_opt_memzero(uint8_t * src, uint32_t length)
{
	if((src != NULL) && (length > MEMORY_NUM_ZERO))                             	/* Check for NULL pointer and length >0 */
	{
...
uint8_t my_opt_reverse(uint8_t * src, uint32_t length)
{
